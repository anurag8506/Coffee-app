...
const Home = ({ userName }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      const id = await AsyncStorage.getItem('USERIDH');
      firestore()
        .collection('cartH')
        .where('addedBy', '==', id)
        .onSnapshot(snapshot => {
          ...
          <View style={styles.header1}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('UserScreen')}>
              <Image
                ...
                source={require('../../assets/Deepu1.png')}
              />
            </TouchableOpacity>
          </View>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('Cart')}>
          ...
          {cartCount > 0 && (
            <View
              ...
              <Text style={{
                color: 'white', alignItems: 'center', fontSize: 25, fontWeight: 'bold', justifyContent: 'center',
                alignContent: 'center',
                alignSelf: 'center',
              }}>{cartCount}</Text>
            </View>
